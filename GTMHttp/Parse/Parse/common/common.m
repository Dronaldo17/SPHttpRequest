...
int convertStingEncoding(const char* toEncoding, const char* fromEncoding, 
    ...
    if (((iconv_t) -1) == handle) {
        apiLogDebug(@"Do not Support Encoding  toEncoding[%s] fromEncoding[%s]", toEncoding, fromEncoding);
...
//    return @"ERROR";
    ...
    apiLogDebug(@"Can not recognize the encoding, use UTF-8 as default");
    free(outBuffer);
    return [NSString stringWithUTF8String:inBuffer];
    
out_free:
    
    apiLogDebug(@"Success Convert from currentEncoding[%s] to UTF8", *currentEncoding);
...
BOOL checkIsURL(NSString* str){
    ...
    apiLogDebug(@"blankSpace ===== %d",[blankSpace count]);
    
    if ([urlStr isMatchedByRegex:regrexUrl] && [NSURL URLWithString:urlStr] && ([blankSpace count]==1)) {
        apiLogDebug(@"It is an url = %@",urlStr);  
        return YES;
    }
    
    apiLogDebug(@"It is not an url = %@",urlStr);
...
NSString* doParseURL(NSString *url){
    ...
    if (!isEmpty(url)) {//判断url字符串不为空
        ...
        if ([url isMatchedByRegex:regrexUrl] && [NSURL URLWithString:url]) {//判断是否为url格式
            ...
            NSLog(@"range_http ===location===%d length===%d",range_http1.location,range_http1.length);
            NSLog(@"range_https ===location===%d length===%d",range_https1.location,range_https1.length);
    ...
    apiLogDebug(@"strURL ====== %@",strURL);
    return strURL;
}
NSString* ageNumber(NSDate* birthDate)
{
    if (!birthDate) {
        return nil;
    }
        ...
        NSString *birthYear = [dateFormat stringFromDate:birthDate];
        NSString *nowYear = [dateFormat stringFromDate:[NSDate date]];
    
        int age = [nowYear intValue] - [birthYear intValue];
        if (age == 0) {
            age = 1;
        }
...
NSString * birthDayDateYYYYMMDD (NSDate * birthDate)
{
    if (!birthDate) {
    return nil;
    }
